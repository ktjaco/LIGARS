...
module.exports = {
	"class": L.Class.extend({
		...
		geocode : function (query, cb, context) {
			Util.jsonp('http://dev.virtualearth.net/REST/v1/Locations', {
				query: query,
				key : this.key
			}, function(data) {
				var results = [];
	...
	"class": L.Class.extend({
		options: {
			...
			htmlTemplate: function(r) {
				...
				if (a.pub || a.internet_cafe || a.gym || a.school || a.restaurant) {
					parts.push('{pub} {internet_cafe} {gym} {school} {restaurant}');
				}				
		...
		geocode: function(query, cb, context) {
			Util.jsonp(this.options.serviceUrl + 'search.php', L.extend({
				q: query,
				limit: 5,
				format: 'json',
				addressdetails: 1
...
module.exports = {
	"class": L.Class.extend({
		options: {
			...
			reverseUrl: 'http://photon.komoot.de/reverse/',
			nameProperties: [
				'name',
				'street',
				'suburb',
